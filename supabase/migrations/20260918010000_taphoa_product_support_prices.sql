-- Expose management Sheet O/P support-price band to the product frontend payload.
-- O = J + 1.5%, P = J + 3%; values stay in the UI's thousand-VND units.
...
as $$
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'giaHoTroTu',case
        when jsonb_typeof(p.raw_row)='array' and coalesce(p.raw_row->>14,'') ~ '^[0-9]+([.,][0-9]+)?$'
          then replace(p.raw_row->>14,',','.')::numeric
        else null
      end,
      'giaHoTroDen',case
        when jsonb_typeof(p.raw_row)='array' and coalesce(p.raw_row->>15,'') ~ '^[0-9]+([.,][0-9]+)?$'
          then replace(p.raw_row->>15,',','.')::numeric
        else null
      end,
      'active',p.is_active,
