-- Allow admins to correct how the selected supermarket price should be interpreted.
-- Raw source snapshot is preserved; compare overrides are stored separately.

alter table public.taphoa_product_media
  add column if not exists market_selected_price_vnd numeric,
  add column if not exists market_compare_kind text,
  add column if not exists market_compare_units_per_carton numeric;

update public.taphoa_product_media
set market_selected_price_vnd = coalesce(market_selected_price_vnd, market_carton_price_vnd::numeric, market_retail_price_vnd)
where market_selected_price_vnd is null;
...
begin
  ...
  if v_candidate_id like 'link:%' then
    ...
    into
      ...
      v_carton_price,v_units,v_label2,v_qty2,v_label3,v_qty3,v_retail_price,v_selected_price
...
begin
  ...
  if v_kind='carton' and (v_qc is null or v_qc<=0) then
    raise exception 'taphoa_market_compare_qc_required' using errcode='22023';
  end if;
  ...
  return jsonb_build_object(
    ...
    'retail_price',case
      when v_kind='carton' and v_selected is not null then v_selected/nullif(v_qc,0)
      else v_selected
    end
  );
end;
