-- Allow a local editor placeholder (SP...) to become one immutable product code in Supabase.
-- Scope: Web <-> Supabase. Sheet acknowledgement for newly-created rows is handled later.
...
begin
  ...
  if not found and v_requested_code ~ '^SP[0-9]+$' then
    -- One placeholder can be retried safely if the browser loses the first response.
    perform pg_advisory_xact_lock(hashtextextended('taphoa_product_code_allocator',0));
    ...
    where sheet_updated_at is null
      ...
      and upper(coalesce(raw_row->>'client_code',''))=v_requested_code
    order by updated_at desc
    limit 1
    for update;

    if found then
      v_code := current_row.product_code;
    else
      v_name := btrim(coalesce(p_product->>'name',''));
      if v_name='' then raise exception 'product_name_required'; end if;

      v_source_key := btrim(coalesce(p_product->>'source_key',''));
      ...
      select
        coalesce(max(case when product_code ~ '^SP-[0-9]+$' then substring(product_code from 4)::bigint end),0),
        count(*) filter (where is_active)
      into v_sp_max,v_active_count
      from public.taphoa_products;

      v_number := greatest(v_sp_max,v_active_count)+1;
      loop
        v_code := 'SP-' || case when length(v_number::text)<6 then lpad(v_number::text,6,'0') else v_number::text end;
        exit when not exists(select 1 from public.taphoa_products where product_code=v_code);
        v_number := v_number+1;
      end loop;

      select coalesce(max(source_row),1)+1 into v_source_row
      from public.taphoa_products
      ...
      insert into public.taphoa_products(
        ...
        v_code,v_source_key,v_source_row,v_name,
        v_input,'carton',null,case when v_input is not null and v_sale is not null then greatest(v_sale-v_input,0) else 0 end,
        v_sale,v_sale,null,null,'',
        case when v_sale is null or v_sale<=0 then 'no_price' else 'available' end,
        case when v_sale is null or v_sale<=0 then 'Chưa có giá' else '' end,
        true,
        jsonb_build_object('origin','web','client_code',v_requested_code),
        null,
        now()
      ...
      return jsonb_build_object(
        ...
        'product_code',v_code,'source_key',v_source_key
      );
    end if;
  ...
  if not v_changed then
    return jsonb_build_object('ok',true,'created',false,'changed',false,'product_code',v_code);
  end if;
  ...
  -- Existing sheet-managed products keep the realtime outbox path.
  if current_row.sheet_updated_at is not null then
  ...
  return jsonb_build_object('ok',true,'created',false,'changed',true,'product_code',v_code,'row_hash',v_hash);
end;
$$;
...
-- A product created on the web has sheet_updated_at = null until it is acknowledged by the Sheet.
-- Do not let an inbound Sheet scan deactivate it before that acknowledgement exists.
...
begin
  ...
  where p.is_active=true
    and p.sheet_updated_at is not null
    and exists (
      select 1
