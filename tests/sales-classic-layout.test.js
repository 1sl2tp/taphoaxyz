...
test('Sales customer selector is one compact visible row without a duplicated Khách label',()=>{
  assert.match(source,/sales-customer-input/);
  assert.match(salesCss,/\.sales-customer-field\{[^}]*display:flex[^}]*align-items:center/s);
  assert.match(salesCss,/\.sales-customer-field>small\{[^}]*display:none/s);
