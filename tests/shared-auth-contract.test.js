...
test('post-login refresh cannot turn a valid session into a login failure',()=>{
  ...
  assert.match(runtime,/try\{\s*await refreshFixedSheets\(\);\s*\}catch\(error\)\{\s*console\.error\('post-login refresh'/);
  const authFailure=runtime.match(/catch\(error\)\{\s*console\.error\('login auth'[\s\S]*?return;\s*\}/)?.[0]||'';
  assert.match(authFailure,/showLoginScreen\(\)/);
...
test('restore keeps a valid session even if UI sheet refresh fails',()=>{
  const runtime=fs.readFileSync(new URL('../src/fixed-production-overrides.js',import.meta.url),'utf8');
