...
test('PWA is branded Bán Hàng and uses separate favicon and app icons', () => {
  ...
  assert.ok(manifest.icons.some(icon => icon.src === './src/assets/app-icon-192-bag-v5.png' && icon.sizes === '192x192' && icon.purpose === 'any'));
  assert.ok(manifest.icons.some(icon => icon.src === './src/assets/app-icon-512-bag-v5.png' && icon.sizes === '512x512' && icon.purpose === 'any'));
  assert.ok(manifest.icons.some(icon => icon.src === './src/assets/app-icon-maskable-512-bag-v5.png' && icon.sizes === '512x512' && icon.purpose === 'maskable'));
