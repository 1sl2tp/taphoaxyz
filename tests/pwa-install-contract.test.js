...
test('PWA is branded Bán Hàng and installable on Android/iOS with square app icon', () => {
  ...
  assert.match(index, /<link\s+rel="icon"\s+type="image\/svg\+xml"\s+href="\.\/src\/assets\/app-icon\.svg">/);
  assert.match(index, /<link\s+rel="shortcut icon"\s+type="image\/svg\+xml"\s+href="\.\/src\/assets\/app-icon\.svg">/);
