...
(function(){
  ...
  function getOrderMeta(){
    ...
    return {orderId:orderId || '--',customer,time:time || '--'};
  }
  ...
  function buildCapture(entries){
    ...
    capture.innerHTML=`
      <div style="padding:22px 28px 18px;background:#f8fafc;border-bottom:1px solid #e5e7eb;">
        <div style="font-size:12px;color:#9ca3af;font-weight:700;">Mã đơn: ${escapeHtml(meta.orderId)}</div>
  ...
  async function shareCartOrderImageV2(){
    ...
    try{
      ...
      const safeId=String(built.meta.orderId||'giohang').replace(/[^a-zA-Z0-9_-]+/g,'_');
      const fileName=(safeId && safeId!=='--' ? 'donhang_'+safeId : 'giohang')+'.png';
  ...
  window.shareCartOrderImage=shareCartOrderImageV2;

  window.shareOrderImage=function(...args){
    if(isCartOpen() && currentCartEntries().length) return shareCartOrderImageV2();
  ...
  if(cartShareBtn){
    cartShareBtn.onclick=shareCartOrderImageV2;
    cartShareBtn.setAttribute('data-cart-share-version','2');
  }
})();
