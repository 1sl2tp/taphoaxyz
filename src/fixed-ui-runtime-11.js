            ...
            for (let ng in sourceSummary) {
                ...
                sumHtml += `
                    <tr class="summary-source-row">
                        <td class="summary-source-name py-3 font-bold"><button type="button" data-source="${escapeProductEditorValue(ng)}" onclick="openSourceDetailFromCell(this, 'dongiao')" class="allow-fast-click source-summary-link text-left font-bold">${escapeProductEditorValue(ng)}</button></td>
                ...
                orderHtml += `
                    <div onclick="clickOrder('${k}', 'dongiao')" class="allow-fast-click bg-white rounded-[16px] p-3.5 shadow-sm border border-gray-100 hover:border-success/50 cursor-pointer transition mb-3">
                        <div class="pointer-events-none flex items-start gap-3 min-w-0 w-full">
                            <div style="width: 36px; height: 36px;" class="order-rank-dot order-rank-dot-delivered rounded-full font-extrabold text-sm flex items-center justify-center shrink-0">${orderKeys.length - idx}</div>
