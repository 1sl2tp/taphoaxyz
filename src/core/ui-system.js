...
function cleanActionText(value=''){return String(value).replace(/[🛒🖼️✅⏳📝🗑️📦✕×◉‹›]/gu,'').trim();}
...
function decorateSalesCleanup(root){
  ...
  if(searchRow&&!searchRow.querySelector('.ui-search-leading-icon')){
    const leading=document.createElement('span');leading.className='ui-search-leading-icon';leading.setAttribute('aria-hidden','true');leading.innerHTML=icon('search',{size:18});searchRow.prepend(leading);
  }
  const quick=root.querySelector?.('[data-screen-id="sales"] .sales-cart-quick');
  if(quick&&!quick.dataset.uiCompact){
    const label=cleanActionText(quick.textContent)||'Trống';quick.replaceChildren();quick.insertAdjacentHTML('afterbegin',icon('cart',{size:18}));
    const copy=document.createElement('span');copy.textContent=label.replace(/^(\d+)\s+(.+)$/,(_,qty,total)=>`${qty} SP · ${total}`);quick.append(copy);quick.dataset.uiCompact='1';
  }
  for(const panel of root.querySelectorAll?.('[data-screen-id="sales"] .sales-cart-panel')||[]){
    ...
    const chip=panel.querySelector('.sales-cart-head>span'),total=panel.querySelector('.sales-cart-total span:last-child b');
    if(chip&&total){const qty=String(chip.textContent||'').trim().replace(/\s*sp$/i,'');chip.textContent=`${qty} SP · ${String(total.textContent||'').trim()}`;panel.dataset.uiCompact='1';}
  }
}
...
function decoratePendingCards(root){
  ...
  for(const card of root.querySelectorAll?.('[data-screen-id="pending"] .pending-order-card')||[]){
    ...
    cleanStatusText(status);addClasses(status,'ui-status','ui-status-pending');
    const primary=document.createElement('span');primary.className='pending-order-primary';addClasses(customer,'pending-order-customer');primary.append(customer);
    const side=document.createElement('span');side.className='pending-order-side';side.append(status,total);top.replaceChildren(primary,side);
    const context=document.createElement('span');context.className='pending-order-context';const line=document.createElement('small');line.className='pending-order-context-line';
    line.textContent=[compactOrderId(card.dataset.orderOpen),String(meta.textContent||'').trim()].filter(Boolean).join(' · ');context.append(line);mid.replaceChildren(context);if(profit)mid.append(profit);card.dataset.uiCard='pending';
  }
}
...
function decorateOverlayChrome(root){
  for(const panel of root.querySelectorAll?.(MODAL_PANELS)||[])addClasses(panel,'ui-modal-surface');
  for(const backdrop of root.querySelectorAll?.(OVERLAY_BACKDROPS)||[])addClasses(backdrop,'ui-overlay-backdrop');
}
...
function decorateActionIcons(root){
  ...
  for(const button of root.querySelectorAll?.('[data-cart-close],[data-detail-close],[data-order-close],[data-print-close],[data-source-close]')||[]){addClasses(button,'ui-icon-button');setButtonIcon(button,'close',{size:18});}
...
export function decorateDetailUi(root,screenId){
  ...
  if(titleNode){
    const fullId=titleNode.dataset.fullOrderId||String(titleNode.textContent||'').trim();if(fullId&&!titleNode.dataset.fullOrderId)titleNode.dataset.fullOrderId=fullId;if(fullId)titleNode.title=fullId;
    if(String(titleNode.textContent||'').trim()!==config.title)titleNode.textContent=config.title;addClasses(titleNode,'order-detail-title');setRole(titleNode,'panel-title');if(context&&fullId)replaceText(context,fullId,compactOrderId(fullId));
  }
...
export function decorateUi(root){
  if(!root)return;
  decorateShellUi(root);decorateSalesCleanup(root);decoratePendingCards(root);decorateOrderStatuses(root);decorateOverlayChrome(root);decorateActionIcons(root);
  ...
  const screen=root.querySelector?.('[data-screen-id]')||root.closest?.('[data-screen-id]');const screenId=screen?.dataset?.screenId;if(screenId)decorateDetailUi(root,screenId);
}

export function installUiSystem(host){
  if(!host)return()=>{};let queued=false;
  const sync=()=>{queued=false;decorateUi(host);};const schedule=()=>{if(queued)return;queued=true;(globalThis.queueMicrotask||((fn)=>Promise.resolve().then(fn)))(sync);};
  const Observer=globalThis.MutationObserver;const observer=Observer?new Observer(schedule):null;observer?.observe(host,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','aria-current','type','disabled']});sync();return()=>observer?.disconnect();
}

if(typeof document!=='undefined'){
  decorateShellUi(document);const host=document.getElementById('appShell')||document.getElementById('screenHost');if(host)installUiSystem(host);
}
