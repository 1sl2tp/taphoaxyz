...
function orderCard(o,index,canViewCost){
  ...
  const codes=items.length||Number(o.tongMa)||0,totalQty=qty||Number(o.tongSL)||0;
  return `<button class="delivered-order-card ui-row" type="button" data-order-open="${esc(o.id)}">
    <div class="delivered-order-top"><span><b>${esc(o.tenKH||'Khách lẻ')}</b><em>Đã giao</em></span><strong>${money(o.tongTien)}</strong></div>
    <div class="delivered-order-mid"><span><small>#${index+1}</small> · ${esc(compactOrderId(o.id))} · ${esc(fmtDate(o.ngay))} · ${codes} mã · ${totalQty} SP</span>${canViewCost?`<strong>+${money(profit)}</strong>`:''}</div>
...
function detailMarkup(o,{canViewCost=false,canManageOrders=false}={}){
  ...
  return `<div class="delivered-overlay" data-order-detail>
    ...
    <section class="delivered-detail-panel ui-popup-l1">
      <header class="ui-popup-header"><strong>Đơn đã giao</strong><span class="ui-status ui-status-success">Đã giao</span><div class="ui-action-group"><button class="ui-action ui-action-secondary" type="button" data-receipt-share>Chia sẻ ảnh</button><button class="ui-icon-button" type="button" data-detail-close aria-label="Đóng chi tiết"></button></div></header>
      <div class="classic-receipt" data-receipt-capture>
        <div class="delivered-detail-meta"><b>KH: ${esc(o.tenKH)}</b> · ${esc(compactOrderId(o.id))} · ${esc(fmtDate(o.ngay))}</div>
        ...
        <div class="delivered-detail-total ui-summary"><b>Tổng ${qty} SP</b><span><strong>${money(o.tongTien)}</strong>${canViewCost?`<small>Lãi +${money(profit)}</small>`:''}</span></div>
      </div>
...
function printMarkup(o){
  const items=o.items||[];return `<div class="delivered-overlay delivered-print-overlay"><button class="delivered-backdrop" type="button" data-print-close aria-label="Đóng"></button><section class="delivered-print-panel ui-popup-l2">
    <header class="ui-popup-header"><strong>Đơn đã giao</strong><div class="ui-action-group"><button class="ui-action ui-action-primary" type="button" data-print-now>In</button><button class="ui-icon-button" type="button" data-print-close aria-label="Đóng bản in"></button></div></header>
    <div class="delivered-print-meta"><b>KH: ${esc(o.tenKH)}</b><span>${esc(compactOrderId(o.id))} · ${esc(fmtDate(o.ngay))}</span></div>
...
function deliveredResultsMarkup(state){
  ...
  return {done,summary:`<h2>Theo nguồn <small>(${done.length} đơn)</small></h2>${summaryMarkup(done,canViewCost)}`,list:done.map((order,index)=>orderCard(order,index,canViewCost)).join('')||'<div class="delivered-empty">Không có đơn trong khoảng này</div>'};
}
